...
| カテゴリ分類記号 | ユーザーがレシートの品目横に手書きする記号（○●△▲）。カテゴリを指示する |
...
**IMG-03** [導出] 手書き記号 ○（白丸）がついた品目は「除外8」（8%税率の除外品）に分類する
...
**IMG-04** [導出] 手書き記号 ●（黒丸）がついた品目は「除外10」（10%税率の除外品）に分類する
...
**IMG-12** [制約] 複数レシート検出時、送信ボタン 1 回で現在表示中のレシート（フォームから取得）＋残りの全レシート（解析結果から直接構築）をまとめて送信する

> 代替案: レシートごとに個別に確認・送信する → 操作の手間が増えるため却下

<details><summary>実装マッピング</summary>

`index.html` — `submitData()` で `dataList` を構築。1 件目は `buildDataFromForm()`、2 件目以降は `buildDataFromReceipt()` で構築し、`sendSingleData()` で順次送信。
